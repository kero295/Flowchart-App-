...
public:
	...
	//Function to delete the statements and connectors from the 
	void DeleteStatement(Statement* selected);
	void DeleteConnector(Connector* selected);
	...
	bool ValidateChart();
	void RunChart();
	// Step-by-step debug run (pauses after each statement and shows variables)
	void DebugRun();
	// Generate corresponding C++ code into the given filename
	void GenerateCode(const string &filename);

	bool IsStatementOverlapping(Point P) const;

private:
	void GenerateCodeFrom(Statement* s, ofstream& ofs, bool visited[]);

};

// inline small accessors to avoid multiple-definition issues when included
inline Input* ApplicationManager::GetInput() const { return pIn; }
inline Output* ApplicationManager::GetOutput() const { return pOut; }
